...
{
  /// <summary>
  /// Concrete implementation of <see cref="BPDMNodeStyleBase"/> for gateway nodes.
  /// </summary>
  public class GatewayNodeStyle : BPDMNodeStyleBase, IStyle
  {
    public Gateway Gateway { get; set; }

    protected override object ResourceKey {
      get { return Gateway.Type; }
    }
    ...
    protected override GeneralPath GetOutline(INode node) {
      ...
      path.MoveTo((layout.GetTopLeft()+layout.GetBottomLeft())/2); // <
      path.LineTo((layout.GetTopLeft()+layout.GetTopRight())/2); // ^
      path.LineTo((layout.GetTopRight() + layout.GetBottomRight()) / 2); // >
      path.LineTo((layout.GetBottomLeft() + layout.GetBottomRight()) / 2); // v
      path.LineTo((layout.GetTopLeft() + layout.GetBottomLeft()) / 2); // <
      return path;
    }
    ...
    public IRow Col
    {
        get
        {
            return this.Gateway.Col;
        }
        set
        {
            this.Gateway.Col = value;
        }
    }
  }
}
